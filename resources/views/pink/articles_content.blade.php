        ...
        @foreach ($articles as $article)
            <div class="sticky hentry hentry-post blog-big group">
                ...
                <div class="meta group">
                    ...
                    <p class="comments"><span><a
                                href="{{ route('articles.show', ['alias' => $article->alias]) }}#respond"
                                title="Comment on Section shortcodes &amp; sticky posts!">{{ count($article->comments) ? count($article->comments) : '0' }}
        ...
        <div class="general-pagination group">
            ...
            @if ($articles->lastPage() > 1)
                ...
                @if ($articles->currentPage() !== 1)
                    <a
                        href="{{ $articles->url($articles->currentPage() - 1) }}">{{ Lang::get('pagination.previous') }}</a>
                @endif
                ...
                @if ($articles->currentPage() !== $articles->lastPage())
                    <a
                        href="{{ $articles->url($articles->currentPage() + 1) }}">{{ Lang::get('pagination.next') }}</a>
                @endif
            @endif
        </div>
    @else
